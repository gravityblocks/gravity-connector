...
- The active relay can dynamically enable, replace, or disable OTLP/HTTP protobuf log export. Export is disabled until the relay supplies a full logs endpoint, level, headers, and resource attributes. The connector fixes the queue at 2,048 records, sends batches of up to 256 every 500 ms, and drops without blocking when the queue is full; protobuf encoding and HTTP run on a dedicated worker. Exporter diagnostics stay in the local sinks and are excluded from OTLP to prevent feedback. Set `logging.otlp.disabled = true` to reject relay requests and keep remote log export permanently disabled.
- Relay-supplied OTLP configuration inherits the relay connection's trust model. The connector validates configuration size and syntax but does not independently authenticate who supplied it; use an HTTPS endpoint to protect the exported logs in transit. Loki 3.x can be targeted through its native `/otlp/v1/logs` endpoint.
