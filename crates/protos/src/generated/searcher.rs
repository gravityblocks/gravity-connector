...
pub mod searcher_service_server {
    ...
    #[async_trait]
    pub trait SearcherService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct SearcherServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> SearcherServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for SearcherServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "searcher.SearcherService";
    impl<T> tonic::server::NamedService for SearcherServiceServer<T> {
